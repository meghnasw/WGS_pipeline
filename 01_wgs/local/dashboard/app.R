    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel(
          ...
          p(
            "This dashboard summarises the main quality control and analysis steps run on the Pseudomonas whole-genome sequencing data."
          ),
          tags$hr(),
          h4("Pipeline overview"),
          tags$ol(
            tags$li(
              strong("FastQC & MultiQC: "),
              "Raw reads are assessed with FastQC, and MultiQC aggregates all reports into a single HTML summary."
            ),
            tags$li(
              strong("Shovill & QUAST: "),
              "Shovill performs genome assembly for each sample. QUAST then evaluates the assemblies and reports metrics such as the number of contigs and N50."
            ),
            tags$li(
              strong("BUSCO: "),
              "BUSCO (Benchmarking Universal Single-Copy Orthologs) estimates genome completeness based on conserved marker genes, reporting percentages of complete, fragmented, and missing BUSCOs."
            ),
            tags$li(
              strong("Prokka: "),
              "Prokka annotates the assemblies, predicting genes and other features. The total gene count per sample is used as a simple annotation summary metric."
            )
          ),
          tags$hr(),
          p(
            "Use the tabs above to explore the output from each step of the pipeline: overall read quality (MultiQC), assembly statistics (Shovill & QUAST), ",
            "genome completeness (BUSCO), annotation metrics (Prokka), and relationships between QC metrics."
          )
        ),
        ...
        tabPanel(
          ...
          h3("MultiQC report"),
          p(
            "This tab shows the aggregated quality control report generated by ",
            strong("MultiQC"),
            ". It combines FastQC and other tool outputs across all samples so you can quickly assess overall read quality."
          ),
          p(
            "Use this report to check for issues such as low base quality, adapter contamination, GC bias, or uneven per-base sequence content ",
            "before looking at downstream assembly and annotation results."
          ),
          tags$hr(),
          tags$iframe(
        ...
        tabPanel(
          ...
          p(
            "Shovill performs the genome assembly, and QUAST summarises assembly quality. ",
            "Here we show the QUAST-derived metrics ",
            strong("number of contigs (contigs)"),
            " and ",
            strong("N50"),
            " for each sample."
          ),
          tags$hr(),
          h4("Assembly metrics plot"),
          p("Faceted bar plot of contigs and N50 per sample, coloured by sample ID (pastel palette)."),
          plotlyOutput("shovill_quast_plot", height = "450px"),
          tags$hr(),
          h4("Assembly metrics table"),
        ...
        tabPanel(
          ...
          p(
            "Prokka annotates the assemblies and reports the total ",
            strong("gene_count"),
            " per sample. This reflects how many genes were predicted on each assembly."
          ),
          tags$hr(),
          h4("Gene count plot"),
          p("Bar plot of Prokka gene counts per sample, coloured by sample ID (pastel palette)."),
        ...
        tabPanel(
          ...
          p(
            "BUSCO evaluates each assembly based on the presence of conserved single-copy orthologs. ",
            "Here we show BUSCO completeness (C%) for each sample, along with fragmented and missing proportions."
          ),
          tags$hr(),
          h4("BUSCO completeness plot"),
          p("Bar plot of BUSCO completeness (C%) per sample, coloured by sample ID."),
          plotlyOutput("busco_plot", height = "350px"),
          tags$hr(),
          h4("BUSCO summary table"),
          p("Table of BUSCO metrics used in this dashboard (completeness, fragmented, missing)."),
        ...
        tabPanel(
          ...
          p(
            "These plots explore how assembly and annotation metrics relate to each other across samples. ",
            "All points are coloured by sample ID using the same pastel palette."
          ),
          tags$hr(),
          h4("Contigs vs N50"),
          p("Scatter plot of number of contigs vs N50 (assembly contiguity)."),
          plotlyOutput("scatter_contigs_n50", height = "350px"),
          tags$hr(),
          h4("N50 vs gene count"),
          p("Scatter plot of N50 vs Prokka gene counts per sample."),
