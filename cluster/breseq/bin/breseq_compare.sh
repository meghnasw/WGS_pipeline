...
if [ -z "$REF_GBK" ]; then
  echo "ERROR: REF_GBK not set. Provide --ref-gbk /path/to/reference.gbk"
  exit 1
fi
...
# Collect all .gd files produced by breseq runs
...
if [[ ! -s "$COMPARE_TSV" ]]; then
  ...
  echo "Skipping COMPARE (already exists): $COMPARE_TSV"
fi

# ANNOTATE (per sample)
while read -r gd; do
  sample=$(basename "$(dirname "$gd")")        # output.gd lives in OUT_ROOT/<sample>/
